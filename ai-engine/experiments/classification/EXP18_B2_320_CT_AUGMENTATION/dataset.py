"""
dataset.py — EXP18 Dataset Loader
"""
...
class CTKidneyDataset18(Dataset):
    def __init__(self, csv_path: str | Path, transform: Optional[Callable] = None, smoke: bool = False, smoke_n: int = 5):
        self.transform = transform
        df = pd.read_csv(csv_path, encoding="utf-8")
        df = df[df["class_name"].isin(CLASS_NAMES)].reset_index(drop=True)
        if smoke: df = (df.groupby("class_name", group_keys=False).apply(lambda g: g.head(smoke_n)).reset_index(drop=True))
    ...
    def class_counts(self) -> dict: return {c: self._counts.get(c, 0) for c in CLASS_NAMES}
