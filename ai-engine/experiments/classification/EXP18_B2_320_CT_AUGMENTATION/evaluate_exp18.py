"""
evaluate_exp18.py — EXP18 External Evaluation
INFERENCE ONLY.
"""
...
from model import build_exp18_model
...
def main():
    ...
    parser.add_argument("--checkpoint", type=str, default=str(_OUT_DIR/"checkpoints"/"best_model_exp18_b2_320_ct_aug.pth"))
    ...
    model = build_exp18_model(cfg); model.load_state_dict(torch.load(ckpt_path, map_location=device)["model_state_dict"])
    model.to(device).eval()

    tf = get_exp18_transforms("test", int(cfg["training"]["input_size"]), cfg)
    ...
    EXP15 = dict(accuracy=0.6293, precision=0.5661, recall=0.8960, specificity=0.3940, f1=0.6938, roc_auc=0.8532)

    print("\n  FAIR COMPARISON:")
    print(f"  {'Metric':15s} | {'Baseline (0.50)':17s} | {'EXP15 Base':14s} | {'EXP18 CT Aug':14s}")
    print("  "+"-"*63)
    ...
    with open(_METRICS_DIR/"external_results.json","w",encoding="utf-8") as f: json.dump({"experiment":"EXP18", "metrics":metrics,"comparison_exp15":EXP15,"baseline":BASE}, f, indent=2)
